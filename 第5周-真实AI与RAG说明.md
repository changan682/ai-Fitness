...
| `mvn -B test` | **131 / 131 ✅** | Java 全量单测（**第 5 周当天的快照**） |
| `pytest` | **40 / 40 ✅** | Python 全量单测（**第 5 周当天的快照**） |

合计 **96 项接口级断言 + 171 项单元测试**全绿（第 5 周当天的快照）。

> ⚠️ **上表的单测数字是第 5 周的历史快照，不要当成当前值**。第 6-8 周与后续体验优化
> 批次持续补测试，截至体验优化批次 D 实际为：**Java 249 项、Python 426 项、前端 36 项**
> （共 711 项）。
> 上表里的 4 个验收脚本断言数（31 / 26 / 27 / 12）到当前仍然准确。
> 另：`chat` 的降级层数已由 3 层扩为 **4 层**（新增"知识库没覆盖 → 大模型通用知识 + `llm_only` 标注"），
> 详见 `python-agent/README.md` §7.2。
