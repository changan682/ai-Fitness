...
public class TrainingBatchRequest {
    ...
    @NotEmpty(message = "训练记录不能为空")
    @Size(max = 20, message = "单次最多录入20条记录")
    @Valid
    private List<TrainingItem> records;
    ...
    public static class TrainingItem {
        ...
        // 规范 2.2 的子项为必填：t_training_record.sets/reps 是 NOT NULL，
        // 若此处允许缺省并用默认值兜底，会把「漏传」静默变成一条脏数据。
